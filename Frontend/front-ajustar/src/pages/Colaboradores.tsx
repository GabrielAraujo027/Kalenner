...
export default function Colaboradores() {
  ...
  const handleAddProfessional = async () => {
    ...
    const { data, error } = await supabase
      .from("collaborators")
      .insert([{ name: professionalName, role: professionalDescription }])
      .select()
      .single();
    ...
    if (error) {
      ...
      if (selectedServices.length > 0 && data) {
        const relations = selectedServices.map((serviceId) => ({
          collaborator_id: data.id,
          service_id: serviceId,
        }));
        await supabase.from("collaborator_services").insert(relations);
      }
    }
  };
  ...
  const handleUpdateCollaborator = async () => {
    ...
    const { error } = await supabase
      .from("collaborators")
      .update({ name: editingCollaborator.name, role: editingCollaborator.role })
  ...
  const handleAddService = async () => {
    ...
    const { error } = await supabase
      .from("services")
      .insert([{ name: serviceName, description: serviceDescription }]);
  ...
  const handleUpdateService = async () => {
    ...
    const { error } = await supabase
      .from("services")
      .update({ name: editingService.name, description: editingService.description })
